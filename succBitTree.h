...
public:	
	...
	inline void Search(succBitNode* a_node, branch* a_rect, int d, vector<int> &result) //B + L
	{
		ASSERT(a_node);
		ASSERT(a_node->m_level >= 0);
		ASSERT(a_rect);
		...
		else // This is a leaf node
		{
			for (int index = 0; index < a_node->m_count; ++index)
			{
				...
				if (edgeSum >= max(a_rect->edge, (int)edge) - d)  // label 
				{
					this->fl->getValue(tmp->idx, begin, end);
					int vertexSum = sv->unionListSumInternal(a_rect->vl, begin, end);

					if (vertexSum + edgeSum >= max(a_rect->vertex, (int)vertex) + max(a_rect->edge, (int)edge) - d) // label filter
					{
						this->fb->getValue(tmp->idx, begin, end);
						sd->decodeLeafList(begin, end, bt, countTuple);
						int sum = a_rect->unionListSum(a_rect->bl, bt, countTuple);
						if (sum >= 2 * max(a_rect->vertex, (int)vertex) - vertexSum - 2 * d) // branch filter
						{
							int *id = (int*)a_node->m_branch[index].child;
							result.push_back(*id);
#if 0
