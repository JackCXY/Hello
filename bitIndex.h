...
public:
	bitmap *bm;       //gama解码数组
	int blockSize;   //F数组块大小
	u64 integerLen;  //F数组长度
	u64 indexSize;  //索引大小

	rankSelect *posRank;  //位置索引PosBit
	rankSelect *flagRank; //标志索引flagBit(1, gamma编码, 0 定长编码)
	 
	u64 *fsuccinct; //F数组压缩后的01串
	u64 fLen; //01串长度
	
	u64 *blockOffset; //block的偏移量
	u64 sampleFSLength; //每个值的编码长度

	u64 *words; //定长编码数组
	u64 wordsLength; //编码words需要的码长
...
public:
	...
	inline u32 unionListSumInternal(BList *bl, const u64 &begin, const u64 &end)
	{
		...
		{
			u64 offset = begin + bl->bt[j].column; //bit串中的偏移量
			if (offset > end)
				continue;
