...
public:
	...
	void Search(Node* a_node, branch* a_rect, int d, vector<int> &result)
	{
		ASSERT(a_node);
		...
		if (a_node->IsInternalNode()) // This is an internal node in the tree
		{
			for (int index = 0; index < a_node->m_count; ++index)
			{
				Node *tmp = (Node *)a_node->m_branch[index].child;
				if (OverlapLabel(a_rect, &a_node->m_branch[index], d))
				{
					if (OverlapBranch(a_rect, &a_node->m_branch[index], d))
						Search((Node *)a_node->m_branch[index].child, a_rect, d, result);
				}
			}
		}
		else // This is a leaf node
		{
			for (int index = 0; index < a_node->m_count; ++index)
			{
				branch *tmp = &a_node->m_branch[index];
				int vertexSum = a_rect->unionListSum(a_rect->vl, tmp->vl);
				if (vertexSum >= max(a_rect->vertex, tmp->vertex) - d)  // label 
				{
					int edgeSum = a_rect->unionListSum(a_rect->el, tmp->el);
					if (vertexSum + edgeSum >= max(a_rect->vertex, tmp->vertex) + max(a_rect->edge, tmp->edge) - d) // label filter
					{
						if (OverlapBranch(a_rect, &a_node->m_branch[index], d)) // branch filter
						{
							//vector<int> dq = bhash->queryDegree(a_rect->bl);
							//vector<int> dg = bhash->queryDegree(tmp->bl);
							int ed = 0;
							//int ed = common::degreeEditDistance(dg, dq);
							if (d >= max(a_rect->vertex, tmp->vertex) - vertexSum + ed) // degree filter
							{
...
public:
	...
	void nodeBitGamaLen(Node *&a_node, vector<int> &vbt, sTuple &bt, vector<int> &vvt, sTuple &vt, vector<int> &vet, sTuple &et, u32 &maxve, u64 &idx,
						u64 &bft, u64 &bgt, u64 &bht, u64 &lft, u64 &lgt, u64 &lht)
	{
		for (int i = 0; i < a_node->m_count; i++)
		{
			int max = 0;
			...
			for (int j = 0; j < a_node->m_branch[i].bl->length; j++)
			{
				...
				if (vbt.size() == BBS)
				{
					bool flag;
					u32 maxValue = 0;
					bt.codeLen += bitOperation::encodingLen(vbt, flag, maxValue, bft, bgt, bht);
					bt.codeNumber += vbt.size();
					if (flag) //true
						bt.gamaCounts++;
					else
			...
			for (int j = 0; j < a_node->m_branch[i].vl->length; j++)
			{
				...
				if (vvt.size() == VBS)
				{
					bool flag;
					u32 maxValue = 0;
					vt.codeLen += bitOperation::encodingLen(vvt, flag, maxValue, lft, lgt, lht);
					vt.codeNumber += vvt.size();
					if (flag) //ture
						vt.gamaCounts++;
					else
			...
			for (int j = 0; j < a_node->m_branch[i].el->length; j++)
			{
				...
				if (vet.size() == EBS)
				{
					bool flag;
					u32 maxValue = 0;
					et.codeLen += bitOperation::encodingLen(vet, flag, maxValue, lft, lgt, lht);
					et.codeNumber += vet.size();
					if (flag) //ture
						et.gamaCounts++;
					else
	...
	{
		assert(a_node);
		nodeBitGamaLen(a_node, vbt, bt, vvt, vt, vet, et, maxve, idx, bft, bgt, bht, lft, lgt, lht);
		if (a_node->IsInternalNode())
		{
			for (int i = 0; i < a_node->m_count; i++)
			{
				Node *tmp = (Node *)a_node->m_branch[i].child;
				treeBitGamaLen(tmp, vbt, bt, vvt, vt, vet, et, maxve, idx, bft, bgt, bht, lft, lgt, lht);
			}
		}
	}
							...
							u64 &bft, u64 &bgt, u64 &bht, u64 &lft, u64 &lgt, u64 &lht)
	{
		...
		idx = 0;
		maxve = 0;
		this->treeBitGamaLen(this->root, vbt, bt, vvt, vt, vet, et, maxve, idx, bft, bgt, bht, lft, lgt, lht);
